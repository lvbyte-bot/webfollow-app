<template>
  <v-list-item :class="{ 'hover-bg': true }">
    ...
    <v-list-item-title class="d-flex align-center">
      <span class="text-truncate mr-2">{{ item.title }}</span>
    ...
    <template v-slot:append>
        <v-btn  class="button" icon="mdi-open-in-new" variant="text"  title="打开原文" :href="item.link"></v-btn>
      <v-btn  class="button" variant="text" icon title="稍后阅读" @click.stop="toggleSaved">

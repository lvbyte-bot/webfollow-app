  ...
  <v-list-item class="magazine-item py-3" v-bind="$attrs">
    <div class="magazine" :class="{ readly: item.isRead }">
      ...
      <div>
        ...
        <div class="magazine-sec" :class="{ nomagazinethumb: !item.thumbnail && !isVideoOrPodcast }">
          ...
          <div v-if="isVideoOrPodcast">
            <v-img :src="item.thumbnail || item.feed?.icon" class="play-preview" cover height="80px">
          ...
          <v-img v-else-if="item.thumbnail" :src="item.thumbnail" cover height="80px">
          </v-img>
        </div>
      </div>
    </div>
